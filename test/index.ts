...
describe("OWN ME CONTRACT TEST", function() {
	...
	describe("Deployment and initial check", () => {
		...
		it("Deployment should assign the total supply of tokens to the owner", async function () {
			expect(await Nude.balanceOf(deployer.address)).to.equal(await Nude.totalSupply());
		});
	})
	...
	describe("Buy NFTs", () => {
		...
		it("Transaction test", async function () {
			...
			await expect(NudeNFT.connect(user1).buyNFT(1, {value: price})).to.be.revertedWith("You cannot buy your own NFT");
			await expect(NudeNFT.connect(user2).buyNFT(1, {value: insufficientPrice})).to.be.revertedWith("Insufficient gwei for purchase");
			await expect(NudeNFT.connect(user2).buyNFT(1, {value: price})).to.emit(
				NudeNFT,
				"BuyNFT"
			).withArgs(
				1,
